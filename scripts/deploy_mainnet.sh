...
DECLARE_OUTPUT=$(starkli declare --account "$STARKNET_ACCOUNT" --private-key $STARKNET_PRIVATE_KEY --rpc https://api.cartridge.gg/x/starknet/mainnet target/dev/beasts_nft_beasts_nft.contract_class.json 2>&1)
...
# Contract deployment
DEPLOY_OUTPUT=$(starkli deploy --account "$STARKNET_ACCOUNT" --private-key $STARKNET_PRIVATE_KEY --rpc https://api.cartridge.gg/x/starknet/mainnet $CLASS_HASH \
    bytearray:str:"$NAME" \
