use core::byte_array::ByteArrayTrait;
use super::beast_manager::BeastManagerTrait;
use super::beast_definitions;
use super::pack::PackableBeast;
use super::utils::felt252_to_byte_array;

#[generate_trait]
pub impl BeastSvgImpl of BeastSvgTrait {
    /// Generates an SVG image for a beast based on its properties
    fn generate_beast_svg(beast: PackableBeast, rank: u16) -> ByteArray {
        let (prefix_name, beast_name, suffix_name) = BeastManagerTrait::get_full_beast_name(beast);
        let beast_attrs = BeastManagerTrait::get_beast_attributes(beast);

        let mut svg: ByteArray = "";
        
        // SVG header
        svg.append(@"<svg xmlns='http://www.w3.org/2000/svg' width='250' height='350' viewBox='0 0 250 350'>");
        
        // Definitions
        svg.append(@"<defs>");
        svg.append(@"<linearGradient id='gold' x1='0%' y1='0%' x2='100%' y2='100%'>");
        svg.append(@"<stop offset='0%' stop-color='#e5d8b2'/>");
        svg.append(@"<stop offset='60%' stop-color='#b79a5e'/>");
        svg.append(@"<stop offset='100%' stop-color='#8a6d3b'/>");
        svg.append(@"</linearGradient>");
        svg.append(@"<linearGradient id='panel' x1='0%' y1='0%' x2='0%' y2='100%'>");
        svg.append(@"<stop offset='0%' stop-color='#2d2d32'/>");
        svg.append(@"<stop offset='100%' stop-color='#1e1e22'/>");
        svg.append(@"</linearGradient>");
        svg.append(@"<path d='M13 11c0-6 9-6 9 0s-9 10.5-9 10.5S4 17 4 11s9-6 9 0Z' stroke-width='3' id='heart' stroke='#ff6b6b' fill='none'/>");
        svg.append(@"<path transform='scale(1.5)' id='bolt' stroke='#ffd166' stroke-width='2' stroke-linejoin='round' d='M6 2 2 9h5l-4 7'/>");
        svg.append(@"<g id='crown' fill='#e6c56e' stroke='#af8a3c' stroke-width='1'>");
        svg.append(@"<path d='M2 14h16l-1.5 4h-13z'/>");
        svg.append(@"<path d='m3 14 2-9 3 5 4-5 3 5 2-5 2 9Z' stroke-linejoin='round' stroke-linecap='round'/>");
        svg.append(@"<circle cx='5' cy='5' r='1.2' fill='#fff3c4'/>");
        svg.append(@"<circle cx='12' cy='5' r='1.2' fill='#fff3c4'/>");
        svg.append(@"<circle cx='17' cy='5' r='1.2' fill='#fff3c4'/>");
        svg.append(@"</g>");
        svg.append(@"<pattern id='pin' width='12' height='12' patternUnits='userSpaceOnUse' patternTransform='rotate(12)'>");
        svg.append(@"<path fill='#1b1b1f' d='M0 0h12v12H0z'/>");
        svg.append(@"<path fill='#242428' opacity='.3' d='M0 0h12v6H0z'/>");
        svg.append(@"</pattern>");
        svg.append(@"<style>");
        svg.append(@".label{fill:#c9c9d1;font-size:9px;letter-spacing:.4px}.valL{fill:#fff;font-size:16px;font-weight:700}");
        svg.append(@"</style>");
        svg.append(@"</defs>");
        
        // Background
        svg.append(@"<rect width='250' height='350' rx='12' fill='url(#pin)'/>");
        svg.append(@"<rect x='4.5' y='4.5' width='241' height='341' rx='9' fill='none' stroke='url(#gold)' stroke-width='3'/>");
        svg.append(@"<rect x='9' y='9' width='232' height='332' rx='7' fill='none' stroke='#fff' stroke-opacity='.05'/>");
        
        // Crown (show if rank 1)
        if rank == 1 {
            svg.append(@"<use href='#crown' x='12' y='12' width='20' height='20'/>");
        }
        
        // Specials and name
        svg.append(@"<text x='125' y='30' text-anchor='middle' style='fill:#b0b0b6;font-size:13px;font-style:italic'>");
        if prefix_name != 0 {
            svg.append(@"\"");
            let prefix_str = felt252_to_byte_array(prefix_name);
            svg.append(@prefix_str);
            svg.append(@" ");
            let suffix_str = felt252_to_byte_array(suffix_name);
            svg.append(@suffix_str);
            svg.append(@"\"");
        }
        svg.append(@"</text>");
        
        svg.append(@"<text x='125' y='52' text-anchor='middle' style='fill:#fff;font-size:18px;font-weight:700;letter-spacing:.7px'>");
        let beast_name_str = felt252_to_byte_array(beast_name);
        svg.append(@beast_name_str);
        svg.append(@"</text>");
        
        // Beast image with clip path
        svg.append(@"<clipPath id='artClip'>");
        svg.append(@"<rect x='60' y='67' width='130' height='130' rx='8'/>");
        svg.append(@"</clipPath>");
        svg.append(@"<rect x='60' y='67' width='130' height='130' rx='8' fill='#141418'/>");
        svg.append(@"<image x='60' y='67' width='130' height='130' image-rendering='pixelated' href='");
        let beast_image = get_beast_png(beast.id);
        svg.append(@beast_image);
        svg.append(@"' clip-path='url(#artClip)'/>");
        svg.append(@"<rect x='60' y='67' width='130' height='130' rx='8' fill='none' stroke='#fff' stroke-opacity='.08' stroke-width='2'/>");
        
        // Stats panels
        svg.append(@"<g transform='translate(18 208)'>");
        svg.append(@"<rect width='65' height='50' rx='5' fill='url(#panel)'/>");
        svg.append(@"<text x='32.5' y='18' text-anchor='middle' class='label'>TIER</text>");
        svg.append(@"<text x='32.5' y='38' text-anchor='middle' class='valL'>");
        svg.append(@format!("{}", beast_attrs.tier));
        svg.append(@"</text>");
        
        svg.append(@"<g transform='translate(75)'>");
        svg.append(@"<rect width='65' height='50' rx='5' fill='url(#panel)'/>");
        svg.append(@"<text x='32.5' y='18' text-anchor='middle' class='label'>LEVEL</text>");
        svg.append(@"<text x='32.5' y='38' text-anchor='middle' class='valL'>");
        svg.append(@format!("{}", beast_attrs.level));
        svg.append(@"</text>");
        svg.append(@"</g>");
        
        svg.append(@"<g transform='translate(150)'>");
        svg.append(@"<rect width='65' height='50' rx='5' fill='url(#panel)'/>");
        svg.append(@"<text x='32.5' y='18' text-anchor='middle' class='label'>TYPE</text>");
        svg.append(@"<text x='32.5' y='38' text-anchor='middle' style='fill:#fff;font-size:14px;font-weight:700'>");
        let beast_type_str = felt252_to_byte_array(beast_attrs.beast_type);
        svg.append(@beast_type_str);
        svg.append(@"</text>");
        svg.append(@"</g>");
        svg.append(@"</g>");
        
        // Power and Health panels
        svg.append(@"<g transform='translate(21.5 265)'>");
        svg.append(@"<rect width='100' height='60' rx='5' fill='url(#panel)'/>");
        svg.append(@"<use href='#bolt' x='75' y='20'/>");
        svg.append(@"<text x='18' y='22' class='label'>POWER</text>");
        svg.append(@"<text x='18' y='46' class='valL'>");
        svg.append(@format!("{}", beast_attrs.power));
        svg.append(@"</text>");
        svg.append(@"</g>");
        
        svg.append(@"<g transform='translate(131.5 265)'>");
        svg.append(@"<rect width='100' height='60' rx='5' fill='url(#panel)'/>");
        svg.append(@"<use href='#heart' x='68' y='20'/>");
        svg.append(@"<text x='18' y='22' class='label'>HEALTH</text>");
        svg.append(@"<text x='18' y='46' class='valL'>");
        svg.append(@format!("{}", beast_attrs.health));
        svg.append(@"</text>");
        svg.append(@"</g>");
        
        // Close SVG
        svg.append(@"</svg>");
        
        svg
    }
    
    /// Generates a complete data URI for the SVG
    fn generate_svg_data_uri(beast: PackableBeast, rank: u16) -> ByteArray {
        let mut data_uri: ByteArray = "";
        data_uri.append(@"data:image/svg+xml,");
        
        // Get the SVG content
        let svg_content = Self::generate_beast_svg(beast, rank);
        
        // Append the SVG content directly
        data_uri.append(@svg_content);
        
        data_uri
    }
}

fn get_tier_color(tier: u8) -> ByteArray {
    if tier == beast_definitions::TIER_1 {
        "#ff6b6b" // Red for Tier 1 (strongest)
    } else if tier == beast_definitions::TIER_2 {
        "#ffd700" // Gold for Tier 2
    } else if tier == beast_definitions::TIER_3 {
        "#c0c0c0" // Silver for Tier 3
    } else if tier == beast_definitions::TIER_4 {
        "#cd7f32" // Bronze for Tier 4
    } else {
        "#808080" // Gray for Tier 5 (weakest)
    }
}

fn get_beast_png(beast_id: u8) -> ByteArray {
    if beast_id == beast_definitions::WARLOCK { get_warlock_svg() }
    else if beast_id == beast_definitions::TYPHON { get_typhon_svg() }
    else if beast_id == beast_definitions::JIANGSHI { get_jiangshi_svg() }
    else if beast_id == beast_definitions::ANANSI { get_anansi_svg() }
    else if beast_id == beast_definitions::BASILISK { get_basilisk_svg() }
    else if beast_id == beast_definitions::GORGON { get_gorgon_svg() }
    else if beast_id == beast_definitions::KITSUNE { get_kitsune_svg() }
    else if beast_id == beast_definitions::LICH { get_lich_svg() }
    else if beast_id == beast_definitions::CHIMERA { get_chimera_svg() }
    else if beast_id == beast_definitions::WENDIGO { get_wendigo_svg() }
    else if beast_id == beast_definitions::RAKSHASA { get_rakshasa_svg() }
    else if beast_id == beast_definitions::WEREWOLF { get_werewolf_svg() }
    else if beast_id == beast_definitions::BANSHEE { get_banshee_svg() }
    else if beast_id == beast_definitions::DRAUGR { get_draugr_svg() }
    else if beast_id == beast_definitions::VAMPIRE { get_vampire_svg() }
    else if beast_id == beast_definitions::GOBLIN { get_goblin_svg() }
    else if beast_id == beast_definitions::GHOUL { get_ghoul_svg() }
    else if beast_id == beast_definitions::WRAITH { get_wraith_svg() }
    else if beast_id == beast_definitions::SPRITE { get_sprite_svg() }
    else if beast_id == beast_definitions::KAPPA { get_kappa_svg() }
    else if beast_id == beast_definitions::FAIRY { get_fairy_svg() }
    else if beast_id == beast_definitions::LEPRECHAUN { get_leprechaun_svg() }
    else if beast_id == beast_definitions::KELPIE { get_kelpie_svg() }
    else if beast_id == beast_definitions::PIXIE { get_pixie_svg() }
    else if beast_id == beast_definitions::GNOME { get_gnome_svg() }
    else if beast_id == beast_definitions::GRIFFIN { get_griffin_svg() }
    else if beast_id == beast_definitions::MANTICORE { get_manticore_svg() }
    else if beast_id == beast_definitions::PHOENIX { get_phoenix_svg() }
    else if beast_id == beast_definitions::DRAGON { get_dragon_svg() }
    else if beast_id == beast_definitions::MINOTAUR { get_minotaur_svg() }
    else if beast_id == beast_definitions::QILIN { get_qilin_svg() }
    else if beast_id == beast_definitions::AMMIT { get_ammit_svg() }
    else if beast_id == beast_definitions::NUE { get_nue_svg() }
    else if beast_id == beast_definitions::SKINWALKER { get_skinwalker_svg() }
    else if beast_id == beast_definitions::CHUPACABRA { get_chupacabra_svg() }
    else if beast_id == beast_definitions::WERETIGER { get_weretiger_svg() }
    else if beast_id == beast_definitions::WYVERN { get_wyvern_svg() }
    else if beast_id == beast_definitions::ROC { get_roc_svg() }
    else if beast_id == beast_definitions::HARPY { get_harpy_svg() }
    else if beast_id == beast_definitions::PEGASUS { get_pegasus_svg() }
    else if beast_id == beast_definitions::HIPPOGRIFF { get_hippogriff_svg() }
    else if beast_id == beast_definitions::FENRIR { get_fenrir_svg() }
    else if beast_id == beast_definitions::JAGUAR { get_jaguar_svg() }
    else if beast_id == beast_definitions::SATORI { get_satori_svg() }
    else if beast_id == beast_definitions::DIREWOLF { get_direwolf_svg() }
    else if beast_id == beast_definitions::BEAR { get_bear_svg() }
    else if beast_id == beast_definitions::WOLF { get_wolf_svg() }
    else if beast_id == beast_definitions::MANTIS { get_mantis_svg() }
    else if beast_id == beast_definitions::SPIDER { get_spider_svg() }
    else if beast_id == beast_definitions::RAT { get_rat_svg() }
    else if beast_id == beast_definitions::KRAKEN { get_kraken_svg() }
    else if beast_id == beast_definitions::COLOSSUS { get_colossus_svg() }
    else if beast_id == beast_definitions::BALROG { get_balrog_svg() }
    else if beast_id == beast_definitions::LEVIATHAN { get_leviathan_svg() }
    else if beast_id == beast_definitions::TARRASQUE { get_tarrasque_svg() }
    else if beast_id == beast_definitions::TITAN { get_titan_svg() }
    else if beast_id == beast_definitions::NEPHILIM { get_nephilim_svg() }
    else if beast_id == beast_definitions::BEHEMOTH { get_behemoth_svg() }
    else if beast_id == beast_definitions::HYDRA { get_hydra_svg() }
    else if beast_id == beast_definitions::JUGGERNAUT { get_juggernaut_svg() }
    else if beast_id == beast_definitions::ONI { get_oni_svg() }
    else if beast_id == beast_definitions::JOTUNN { get_jotunn_svg() }
    else if beast_id == beast_definitions::ETTIN { get_ettin_svg() }
    else if beast_id == beast_definitions::CYCLOPS { get_cyclops_svg() }
    else if beast_id == beast_definitions::GIANT { get_giant_svg() }
    else if beast_id == beast_definitions::NEMEANLION { get_nemeanlion_svg() }
    else if beast_id == beast_definitions::BERSERKER { get_berserker_svg() }
    else if beast_id == beast_definitions::YETI { get_yeti_svg() }
    else if beast_id == beast_definitions::GOLEM { get_golem_svg() }
    else if beast_id == beast_definitions::ENT { get_ent_svg() }
    else if beast_id == beast_definitions::TROLL { get_troll_svg() }
    else if beast_id == beast_definitions::BIGFOOT { get_bigfoot_svg() }
    else if beast_id == beast_definitions::OGRE { get_ogre_svg() }
    else if beast_id == beast_definitions::ORC { get_orc_svg() }
    else if beast_id == beast_definitions::SKELETON { get_skeleton_svg() }
    else {
        // Default empty image
        "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg=="
    }
}

// Include all the beast PNG functions// Auto-generated beast SVG functions

fn get_ammit_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_anansi_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_balrog_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_banshee_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_basilisk_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_bear_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_behemoth_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_berserker_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_bigfoot_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_chimera_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_chupacabra_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_colossus_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_cyclops_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_direwolf_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_dragon_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_draugr_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_ent_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_ettin_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_fairy_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_fenrir_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_ghoul_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_giant_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_gnome_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_goblin_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_golem_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_gorgon_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_griffin_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_harpy_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_hippogriff_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_hydra_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_jaguar_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_jiangshi_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_jotunn_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_juggernaut_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_kappa_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_kelpie_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_kitsune_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_kraken_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_leprechaun_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_leviathan_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_lich_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_manticore_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_mantis_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_minotaur_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_nemeanlion_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_nephilim_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_nue_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_ogre_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_oni_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_orc_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_pegasus_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_phoenix_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_pixie_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_qilin_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_rakshasa_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_rat_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_roc_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_satori_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_skeleton_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_skinwalker_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_spider_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_sprite_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_tarrasque_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_titan_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_troll_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_typhon_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_vampire_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_warlock_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_wendigo_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_weretiger_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_werewolf_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_wolf_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_wraith_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_wyvern_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

fn get_yeti_svg() -> ByteArray {
    let mut svg: ByteArray = "";
    svg.append(@"data:image/png;base64,");
    svg
}

