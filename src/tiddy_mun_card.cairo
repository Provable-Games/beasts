...
#[derive(Copy, Drop)]
pub struct TiddyMunCardAttributes {
    pub tier: u8,
    pub level: u16,
...
#[generate_trait]
pub impl TiddyMunCardImpl of TiddyMunCardTrait {
    /// Generates the same SVG card used by the original Beast collection.
    fn generate_svg(
        prefix_name: felt252,
        suffix_name: felt252,
        beast_name: felt252,
        rank: u16,
        beast_attrs: TiddyMunCardAttributes,
        beast_image: ByteArray,
    ) -> ByteArray {
        let is_shiny = beast_attrs.shiny;
        ...
        svg.append(@"</linearGradient>");
        // Rotating rainbow used for shiny frame
        svg
            .append(
                @"<linearGradient id='shinyRim' gradientUnits='userSpaceOnUse' x1='0' y1='0' x2='250' y2='0' spreadMethod='repeat'>",
            );
        svg.append(@"<stop offset='0%' stop-color='#73FF73'/>");
        svg.append(@"<stop offset='16.67%' stop-color='#FFFF73'/>");
        svg.append(@"<stop offset='33.33%' stop-color='#FFBE73'/>");
        svg.append(@"<stop offset='50%' stop-color='#FF7373'/>");
        svg.append(@"<stop offset='66.67%' stop-color='#C073DC'/>");
        svg.append(@"<stop offset='83.33%' stop-color='#7373FF'/>");
        svg.append(@"<stop offset='100%' stop-color='#73FF73'/>");
        svg
            .append(
                @"<animateTransform attributeName='gradientTransform' type='rotate' from='0 125 175' to='360 125 175' dur='6s' repeatCount='indefinite'/>",
            );
        svg.append(@"</linearGradient>");
        ...
        // Logo gradients and indirection (based on shiny)
        svg.append(@"<linearGradient id='logo_pastel_rainbow' x1='0%' y1='0%' x2='0%' y2='100%'>");
        svg.append(@"<stop offset='0%' stop-color='#73FF73'/>");
        svg.append(@"<stop offset='20%' stop-color='#FFFF73'/>");
        svg.append(@"<stop offset='40%' stop-color='#FFBE73'/>");
        svg.append(@"<stop offset='60%' stop-color='#FF7373'/>");
        ...
        svg.append(@"<rect x='4' y='6' width='10' height='8' rx='1'/>");
        svg.append(@"<rect x='2' y='7' width='2' height='3'/>");
        svg.append(@"<rect x='14' y='7' width='2' height='3'/>");
        svg.append(@"<rect x='6' y='13' width='6' height='4'/>");
        svg.append(@"</g>");
        svg
            .append(
      ...
      text {
        ...
        if rank == 1 {
            ...
            svg
                .append(
                    @"<text x='227' y='43' text-anchor='middle' style='fill:#b0b0b6;font-size:19px;'>",
                );
        ...
        svg.append(@"<rect width='144' height='144' rx='8'/>");
        svg.append(@"</clipPath>");
        ...
        // Stats panels
        svg.append(@"<g transform='translate(15 210)'>");
        ...
        svg.append(@"<text x='35' y='37' text-anchor='middle' class='valL'>");
        ...
        svg.append(@"<g transform='translate(15 265)'>");
        svg.append(@"<rect width='107' height='65' rx='5' fill='url(#panel)'/>");
        svg.append(@"<text x='35' y='22' text-anchor='middle' class='label'>POWER</text>");
        ...
        svg.append(@"<g transform='translate(128 265)'>");
        svg.append(@"<rect width='107' height='65' rx='5' fill='url(#panel)'/>");
        svg.append(@"<text x='35' y='22' text-anchor='middle' class='label'>HEALTH</text>");
        svg.append(@"<text x='35' y='46' text-anchor='middle' class='valL'>");
        ...
        svg.append(@"</g>");
        svg.append(@"</g>");
        // Draw outer frame last for top layering
        if is_shiny {
            svg
                .append(
                    @"<rect x='4.5' y='4.5' width='241' height='341' rx='9' fill='none' stroke='url(#shinyRim)' stroke-width='4'/>",
                );
        } else {
            svg
                .append(
                    @"<rect x='4.5' y='4.5' width='241' height='341' rx='9' fill='none' stroke='url(#gold)' stroke-width='4'/>",
                );
        }
        svg.append(@"</svg>");
        svg
